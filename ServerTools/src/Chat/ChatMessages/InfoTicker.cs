...
namespace ServerTools
{
    public class InfoTicker
    {
        ...
        public static void SevenDayBuff()
        {
            ulong _worldTime = GameManager.Instance.World.GetWorldTime();
            int _daysUntil7 = 7 - GameUtils.WorldTimeToDays(_worldTime) % 7;
            int gamehour = GameUtils.WorldTimeToHours(_worldTime);

            if (_daysUntil7 == 7 && gamehour < 22)
            {
                List<ClientInfo> _cInfoList = ConnectionManager.Instance.GetClients();
                foreach (ClientInfo myplayer in _cInfoList)
                {
                    string mymessage = "There's something unsettling about today.";
                    myplayer.SendPackage(new NetPackageGameMessage(string.Format("[FF8000]{0}[-]", mymessage), "Server"));
                    myplayer.SendPackage(new NetPackageConsoleCmdClient("buff daySevenWarning", true));
                }
            }
            else {
                List<ClientInfo> _cInfoList = ConnectionManager.Instance.GetClients();
                foreach (ClientInfo myplayer in _cInfoList)
                {
                    ulong _worldTimex = GameManager.Instance.World.GetWorldTime();
                    int dayx = GameUtils.WorldTimeToDays(_worldTimex);
                    int z = 0;
                    z = dayx % 7;
                    if (z != 0)
                    {
                        while (z > 0)
                        {
                            ...
                            if (z == 0)
                            {
                                string mymessage = "The next 7th day horde will be on Day " + dayx;
                                //AdminChat.SendAll(myplayer, mymessage);
                                myplayer.SendPackage(new NetPackageGameMessage(string.Format("[FF8000]{0}[-]", mymessage), "Server"));
                            }
                        }
                    }
                    string strplayer = myplayer.playerName;
                    myplayer.SendPackage(new NetPackageConsoleCmdClient("debuff daySevenWarning", true));
                }
            }
        }
    }
