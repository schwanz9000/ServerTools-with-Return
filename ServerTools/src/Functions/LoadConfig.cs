    ...
    public class Config
    {
                    ...
                    foreach (XmlNode subChild in childNode.ChildNodes)
                    {
                        ...
                        switch (_line.GetAttribute("Name"))
                        {
                            ...
                            case "SetHome":
                                ...
                                break;
                            case "SetReturn":
                                if (!_line.HasAttribute("DelayBetweenSetReturnUses"))
                                {
                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring SetReturn entry because of missing 'DelayBetweenSetReturnUses' attribute: {0}", subChild.OuterXml));
                                    continue;
                                }
                                if (!int.TryParse(_line.GetAttribute("DelayBetweenSetReturnUses"), out TeleportReturn.DelayBetweenUses))
                                {
                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring SetReturn entry because of invalid (non-numeric) value for 'DelayBetweenSetReturnUses' attribute: {0}", subChild.OuterXml));
                                    continue;
                                }
                                if (!_line.HasAttribute("Enable"))
                                {
                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring SetReturn entry because of missing 'Enable' attribute: {0}", subChild.OuterXml));
                                    continue;
                                }
                                if (!bool.TryParse(_line.GetAttribute("Enable"), out TeleportReturn.IsEnabled))
                                {
                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring SetReturn entry because of invalid (true/false) value for 'Enable' attribute: {0}", subChild.OuterXml));
                                    continue;
                                }
                                break;
                            case "CustomCommands":
        ...
        public static void UpdateConfig()
        {
            ...
            using (StreamWriter sw = new StreamWriter(_filepath))
            {
                ...
                sw.WriteLine(string.Format("        <Tool Name=\"SetReturn\" Enable=\"{0}\" DelayBetweenSetReturnUses=\"{1}\" />", TeleportReturn.IsEnabled, TeleportReturn.DelayBetweenUses));
